...
class GEXFWriter(GEXF):
    ...
    def alter_graph_mode_timeformat(self, start_or_end):
        ...
        if not 'timeformat' in self.graph_element.keys():
            if start_or_end is not None:
                if type(start_or_end) == str or type(start_or_end) == unicode:
                    timeformat = 'date'
                    ...
                    raise nx.NetworkXError("start or end attributes must be `int`, `str`, or `float`")
